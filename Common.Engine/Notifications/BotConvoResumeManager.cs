...
public class BotConvoResumeManager : IBotConvoResumeManager
{
    ...
    private readonly IConversationResumeHandler<string> _conversationResumeHandler;
    ...
    public async Task ResumeConversation(string upn)
    {
        ...
        if (graphUser?.Id != null)
        {
            ...
            if (_botConversationCache.ContainsUserId(graphUser.Id))
            {
                ...
                var (data, card) = await _conversationResumeHandler.LoadDataAndResumeConversation(upn);

                var resumeActivity = MessageFactory.Attachment(card);

                await ((CloudAdapter)_adapter)
                    .ContinueConversationAsync(_config.GraphConfig.ClientId, previousConversationReference,
